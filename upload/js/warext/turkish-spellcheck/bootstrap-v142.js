(() => {
  'use strict';
  if (window.__warextTurkishSpellBootstrapV142) return;
  window.__warextTurkishSpellBootstrapV142 = true;
  ...
  async function start() {
    ...
    try {
      await loadScript('dictionary-v142.js', () => !!window.WarextTurkishSpellEngineV142);
      if (!window.WarextTurkishSpellEngineV142) throw new Error('dictionary did not initialize');
      await loadScript('editor-v142.js', () => !!window.__warextTurkishSpellCheckV142);
      document.documentElement.dataset.wtscStatus='assets-ready';
      observer?.disconnect();
    } catch (e) {
      console.error('[Warext Turkish Spell Check] asset load failed', e);
      showAssetError();
    }
  }

  function detect(root=document) { if (editorExists(root)) start(); }
    ...
    observer=new MutationObserver(mutations=>{
      for(const mutation of mutations){
        for(const node of mutation.addedNodes){
          if(node instanceof Element && editorExists(node)){ start(); return; }
        }
      }
    });
