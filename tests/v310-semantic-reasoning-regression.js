...
assert.equal(e.stats.semanticReasoningLayer,'v311-local-graph-calibration');
assert.equal(e.stats.semanticCalibrationVersion,'3.1.1');
...
const lacksPattern = (text,pattern) => !e.analyzeMeaningGraph(text).warnings.some(item => pattern.test(item.rule || ''));

assert.ok(has('Kapı kapalıydı. Bir süre bekledim. Kapı açıktı.','v310-graph-state-contradiction') || has('Kapı kapalıydı. Bir süre bekledim. Kapı açıktı.','v311-entity-scoped-state-contradiction'));
assert.ok(lacks('Kapı kapalıydı. Sonra görevli kapıyı açtı. Kapı açıktı.','v311-entity-scoped-state-contradiction'));
assert.ok(lacks('Dün sunucu kapalıydı. Bugün sunucu açık.','v311-entity-scoped-state-contradiction'));
assert.ok(has('Dosya silindi. Dosya hâlâ mevcuttu.','v310-graph-state-contradiction') || has('Dosya silindi. Dosya hâlâ mevcuttu.','v311-entity-scoped-state-contradiction'));
assert.ok(lacks('Dosya silindi. Sonra dosya yeniden oluşturuldu ve mevcut oldu.','v311-entity-scoped-state-contradiction'));
assert.ok(has('Hiçbir kitabım yoktu. Masaya üç tane kitabımı koydum.','v310-graph-existence-quantity-conflict'));
assert.ok(has('Depoda beş kutu vardı. Depoda dokuz kutu vardı.','v310-graph-quantity-conflict') || has('Depoda beş kutu vardı. Depoda dokuz kutu vardı.','v311-entity-scoped-quantity-conflict'));
assert.ok(lacksPattern('Belki depoda üç kutu vardır. Belki depoda beş kutu olabilir.',/(?:v310-graph-quantity-conflict|v311-entity-scoped-quantity-conflict)/u));
assert.ok(lacks('Depoda beş kutu vardı. Sonra dört kutu daha eklendi. Depoda dokuz kutu vardı.','v311-entity-scoped-quantity-conflict'));
assert.ok(lacksPattern('Eğer kapı kapalıysa bekle. Kapı açıksa içeri gir.',/(?:v310-graph-state-contradiction|v311-entity-scoped-state-contradiction)/u));
assert.ok(lacksPattern('Belki sunucu kapalıdır. Sunucu açık olabilir.',/(?:v310-graph-state-contradiction|v311-entity-scoped-state-contradiction)/u));
assert.ok(has('Sunucu kapalıydı. Sonra hava değişti. Sunucu açıktı.','v311-entity-scoped-state-contradiction'));
...
assert.ok(has('Ali Ahmet ile konuştu. O daha sonra dosyayı açtı.','v311-reference-ambiguous-pronoun'));
assert.ok(lacks('Ali dosyayı hazırladı. Ben dosyayı gönderdim.','v311-reference-ambiguous-pronoun'));
...
assert.equal(graph.version,'3.1.1');
assert.equal(graph.calibrationLayer,'v311-entity-scoped-reasoning');
...
assert.equal(report.reasoningLayer,'v311-local-graph-calibration');
...
assert.equal(paragraph.reasoningLayer,'v311-local-graph-calibration');
...
console.log('V3.1.1 yerel önerme grafiği, varlık kapsamlı geçiş, varsayım ayrımı ve gönderim regresyon testleri başarılı.');